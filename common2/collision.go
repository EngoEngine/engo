...
func (sc *SpaceComponent) SetCenter(p math2d.Point) {
	xDelta := sc.Width / 2
	yDelta := sc.Height / 2
...
// top-left point (this avoids doing the same math each time in your systems)
func (sc *SpaceComponent) Center() math2d.Point {
	xDelta := sc.Width / 2
	yDelta := sc.Height / 2
	p := sc.Position
	if sc.Rotation == 0 {
		return math2d.Point{X: p.X + xDelta, Y: p.Y + yDelta}
	}
	sin, cos := math.Sincos(sc.Rotation * math.Pi / 180)
	xDelta = (sc.Width*cos - sc.Height*sin) / 2
	yDelta = (sc.Height*cos + sc.Width*sin) / 2
...
// rotation of the Component - it may very well be that the Minimum as given by math2d.AABB, is smaller than the Position
...
func (sc SpaceComponent) AABB() math2d.AABB {
	if sc.Rotation == 0 {
		return math2d.AABB{
			Min: sc.Position,
			Max: math2d.Point{X: sc.Position.X + sc.Width, Y: sc.Position.Y + sc.Height},
		}
	}
	...
	return math2d.AABB{Max: math2d.Point{X: xMin, Y: yMin}, Min: math2d.Point{X: xMax, Y: yMax}}
}
...
func (sc SpaceComponent) Corners() (points [4]math2d.Point) {
...
func (sc SpaceComponent) Contains(p math2d.Point) bool {
	points := sc.Corners()
	...
	for i := 0; i < 4; i++ {
		for j := i + 1; j < 4; j++ {
			if t := triangleArea(points[i], points[j], p); t > halfArea || math2d.FloatEqual(t, halfArea) {
				return false
			}
		}
	}
...
func triangleArea(p1, p2, p3 math2d.Point) float32 {
...
// Type implements the math2d.Message interface
...
func (c *CollisionSystem) Update(dt float32) {
	for i1, e1 := range c.entities {
		...
		offset := math2d.Point{X: e1.CollisionComponent.Extra.X / 2, Y: e1.CollisionComponent.Extra.Y / 2}
		...
		for i2, e2 := range c.entities {
			...
			offset = math2d.Point{X: e2.CollisionComponent.Extra.X / 2, Y: e2.CollisionComponent.Extra.Y / 2}
			...
			if IsIntersecting(entityAABB, otherAABB) {
				if cgroup&c.Solids > 0 {
					...
					if e2.CollisionComponent.Main&e1.CollisionComponent.Group&c.Solids != 0 {
						...
						message.Mailbox.Dispatch(CollisionMessage{Entity: e2, To: e1, Groups: cgroup})
					} else {
				...
				message.Mailbox.Dispatch(CollisionMessage{Entity: e1, To: e2, Groups: cgroup})
...
// IsIntersecting tells if two math2d.AABBs intersect.
func IsIntersecting(rect1 math2d.AABB, rect2 math2d.AABB) bool {
...
func MinimumTranslation(rect1 math2d.AABB, rect2 math2d.AABB) math2d.Point {
	mtd := math2d.Point{}
