...
func TestSpaceComponent_Contains(t *testing.T) {
	...
	pass := []math2d.Point{
		math2d.Point{X: 10, Y: 10},
		math2d.Point{X: 50, Y: 50},
		math2d.Point{X: 10, Y: 50},
		math2d.Point{X: 99, Y: 99},
	}
	fail := []math2d.Point{
		// Totally not within:
		math2d.Point{X: -10, Y: -10},
		math2d.Point{X: 120, Y: 120},

		// Only one axis within:
		math2d.Point{X: 50, Y: 120},
		math2d.Point{X: 120, Y: 50},

		// On the edge:
		math2d.Point{X: 0, Y: 0},
		math2d.Point{X: 0, Y: 50},
		math2d.Point{X: 0, Y: 100},
		math2d.Point{X: 50, Y: 0},
		math2d.Point{X: 50, Y: 100},
		math2d.Point{X: 100, Y: 0},
		math2d.Point{X: 100, Y: 50},
		math2d.Point{X: 100, Y: 100},
	}
...
func TestSpaceComponent_Corners(t *testing.T) {
	...
	exp1 := [4]math2d.Point{math2d.Point{X: 0, Y: 0}, math2d.Point{X: 1, Y: 0}, math2d.Point{X: 0, Y: 1}, math2d.Point{X: 1, Y: 1}}
	act1 := space1.Corners()
	for i := 0; i < 4; i++ {
	...
	exp2 := [4]math2d.Point{math2d.Point{X: 0, Y: 0}, math2d.Point{X: 0, Y: 1}, math2d.Point{X: -1, Y: 0}, math2d.Point{X: -1, Y: 1}}
	act2 := space2.Corners()
	for i := 0; i < 4; i++ {
...
func Test_GroupSolid(t *testing.T) {
	...
	CE := func(m, g CollisionGroup) collisionEntity {
		...
		return collisionEntity{
			...
			SpaceComponent: &SpaceComponent{math2d.Point{X: 10, Y: 10}, 50, 50, 0},
		}
	}
