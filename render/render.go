...
type RenderComponent struct {
	...
	Hidden bool
	// Scale is the scale at which to render, in the X and Y axis. Not defining Scale, will default to math2d.Point{1, 1}
	Scale math2d.Point
...
func (rs *RenderSystem) Add(basic *ecs.BasicEntity, render *RenderComponent, space *common2.SpaceComponent) {
...
func (rs *RenderSystem) Update(dt float32) {
	...
	for _, e := range rs.entities {
		...
		if e.RenderComponent.Scale.X == 0 && e.RenderComponent.Scale.Y == 0 {
			e.RenderComponent.Scale = math2d.Point{X: 1, Y: 1}
		}
