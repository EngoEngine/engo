...
type PolylineObject struct {
	...
	Points string
	// LineBounds is the list of math2d.Line objects generated from the points string
	LineBounds []*math2d.Line
}
...
// Bounds returns the level boundaries as an math2d.AABB object
func (l *Level) Bounds() math2d.AABB {
	switch l.Orientation {
	case orth:
		return math2d.AABB{
			Min: l.screenPoint(math2d.Point{X: 0, Y: 0}),
			Max: l.screenPoint(math2d.Point{X: float32(l.width), Y: float32(l.height)}),
		}
	case iso:
		xMin := l.screenPoint(math2d.Point{X: 0, Y: float32(l.height)}).X + float32(l.TileWidth)/2
		xMax := l.screenPoint(math2d.Point{X: float32(l.width), Y: 0}).X + float32(l.TileWidth)/2
		yMin := l.screenPoint(math2d.Point{X: 0, Y: 0}).Y
		yMax := l.screenPoint(math2d.Point{X: float32(l.width), Y: float32(l.height)}).Y + float32(l.TileHeight)/2
		return math2d.AABB{
			Min: math2d.Point{X: xMin, Y: yMin},
			Max: math2d.Point{X: xMax, Y: yMax},
		}
	}
	return math2d.AABB{}
}
...
func (l *Level) mapPoint(screenPt math2d.Point) math2d.Point {
	switch l.Orientation {
	case orth:
		screenPt.Multiply(math2d.Point{X: 1 / float32(l.TileWidth), Y: 1 / float32(l.TileHeight)})
		return screenPt
	case iso:
		return math2d.Point{
	...
	return math2d.Point{X: 0, Y: 0}
}

// screenPoint returns the screen point of the passed in map point
func (l *Level) screenPoint(mapPt math2d.Point) math2d.Point {
	switch l.Orientation {
	case orth:
		mapPt.Multiply(math2d.Point{X: float32(l.TileWidth), Y: float32(l.TileHeight)})
		return mapPt
	case iso:
		return math2d.Point{
...
func (l *Level) GetTile(pt math2d.Point) *Tile {
	mp := l.mapPoint(pt)
	...
	for _, sheet := range sheets {
		...
		for i := 0; i < totalTiles; i++ {
			...
			t.Image = &Texture{
				...
				width:  tw,
				height: th,
				viewport: math2d.AABB{
					Min: math2d.Point{X: u, Y: v},
					Max: math2d.Point{X: u2, Y: v2},
				},
			}
			tileset[curGid] = t
			curGid++
		...
		for i := 0; i < lvl.height; i++ {
			for x := 0; x < lvl.width; x++ {
				...
				if tileIdx := int(mapping[idx]); tileIdx >= 1 {
					...
					t.Point = lvl.screenPoint(math2d.Point{X: float32(x), Y: float32(i)})
				}
