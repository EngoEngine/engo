...
func createLevelFromTmx(tmxBytes []byte, tmxURL string) (*Level, error) {
	...
	for i, tts := range tmxLevel.Tilesets {
		...
		for _, t := range tts.Tiles {
			tile := Tile{}
			tile.Image = &Texture{t.Image.Texture, t.Image.Width, t.Image.Height, math2d.AABB{Max: math2d.Point{X: 1.0, Y: 1.0}}}
			tile.Point = math2d.Point{X: 0, Y: 0}
	...
	for _, tmxImageLayer := range tmxLevel.ImageLayers {
		...
		// create image tile
		imageTile := &Tile{
			math2d.Point{X: float32(tmxImageLayer.X), Y: float32(tmxImageLayer.Y)},
			curImg,
		}
...
func pointStringToLines(str string, xOff, yOff float64) []*math2d.Line {
	...
	lines := make([]*math2d.Line, len(floatPts)-1)
	...
	for i := 0; i < len(floatPts)-1; i++ {
		...
		p1 := math2d.Point{X: x1, Y: y1}
		p2 := math2d.Point{X: x2, Y: y2}
		newLine := &math2d.Line{P1: p1, P2: p2}
