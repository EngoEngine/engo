...
func NewTextureSingle(img Image) Texture {
	id := UploadTexture(img)
	return Texture{id, float32(img.Width()), float32(img.Height()), math2d.AABB{Max: math2d.Point{X: 1.0, Y: 1.0}}}
}
...
func LoadedSprite(url string) (*Texture, error) {
	...
	return &Texture{img.Texture, img.Width, img.Height, math2d.AABB{Max: math2d.Point{X: 1.0, Y: 1.0}}}, nil
}
