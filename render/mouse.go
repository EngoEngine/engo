...
func (m *MouseSystem) Add(basic *ecs.BasicEntity, mouse *MouseComponent, space *common2.SpaceComponent, render *RenderComponent) {
	...
	for _, e := range m.entities {
		...
		*e.MouseComponent = MouseComponent{
		...
		if e.MouseComponent.Track || e.MouseComponent.startedDragging ||
			e.SpaceComponent.Contains(math2d.Point{X: mx, Y: my}) {
